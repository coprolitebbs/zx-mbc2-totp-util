...
#define int32_t int
#define size_t unsigned long  //or uint?
...
//get totp function; steps - current Unix time
char* getCode(uint8_t* hmacKey, int keyLength, long steps);

//decode base32 string function
...
char* getCode(uint8_t* hmacKey, int keyLength, long steps) {
	...
	//s_write(_byteArray, 8);
	_hash = resultHmac();
	
	// STEP 2, apply dynamic truncation to obtain a 4-bytes string
	_offset = _hash[20 - 1] & 0xF; 
	_truncatedHash = (_hash[_offset]  & 0x7f) << 24
           | (_hash[_offset+1] & 0xff) << 16
           | (_hash[_offset+2] & 0xff) <<  8
           | (_hash[_offset+3] & 0xff) ;
...
uint8_t decode_base32(uint8_t* key){
	...
	for(size_t pos = 0; pos <= (strlen((char *)key) - 8); pos += 8){
